        ...
        return {
            ...
            addTransaction(txn, defaultSender) {
                promiseChain = promiseChain.then(async () => {
                    // If txn is already a TransactionWithSigner, use it directly
                    if (txn && typeof txn === 'object' && 'txn' in txn && 'signer' in txn) {
                        console.log('📦 addTransaction: txn is already wrapped with signer');
                        return atc.addTransaction(txn);
                    }
                    // Otherwise wrap it
                    console.log('📦 addTransaction: wrapping txn with signer');
                    console.log('  - txn type:', typeof txn);
                    console.log('  - defaultSender:', defaultSender);
                    console.log('  - defaultSender.signer type:', defaultSender?.signer ? typeof defaultSender.signer : 'undefined');
                    const wrapped = await algokit.getTransactionWithSigner(txn, defaultSender ?? client.sender);
                    console.log('  - wrapped successfully');
                    return atc.addTransaction(wrapped);
                });
                return this;
            },
            async atc() {
